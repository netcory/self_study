...
WITH A AS
(
    SELECT [Reference], CONCAT('[[Name=', Name, ',', 'YieldCurveName=', YieldCurveName, ',',
                                'Type=', Type, ',', 'Model=', Model,
                                ']]')) AS Assets
    FROM [MarketRisk].[dbo].[AID_FastValProduct_Assets]
)
    ...
    SELECT DISTINCT [Reference]
        ,(SELECT CONCAT('[', STUFF((
                        SELECT CONCAT(',[Name=', Name, ',', 'Value=', Value,
                                        ']')
                        FROM [dbo].[AID_FastValProduct_Macros] AS D
                        WHERE D.[Reference] = E.[Reference]
                        
                        FOR XML PATH('')
                        ),1,1,'')
                        ,',[Name=ma_size,Value=40],[Name=threshold1,Value=90%],[Name=threshold2,Value=110%]]'))
